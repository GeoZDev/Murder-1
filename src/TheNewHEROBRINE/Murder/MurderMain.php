...
class MurderMain extends PluginBase {
    ...
    public function onEnable() {
        ...
        $this->config = new Config($this->getDataFolder() . "config.yml", Config::YAML, [
            "join" => TextFormat::BLUE . "{player}" . TextFormat::WHITE . " has joined the match!",
            "quit" => TextFormat::BLUE . "{player}" . TextFormat::WHITE . " has left the match!",
            "countdown" => 40,
            "maxGameTime" => 1200,
            "hub" => "MurderHub"]
        );
        $this->countdown = $this->getConfig()->get("countdown", 40);
        $this->joinMessage = $this->getConfig()->get("join", TextFormat::BLUE . "{player}" . TextFormat::WHITE . " has joined the match!");
        $this->quitMessage = $this->getConfig()->get("quit", TextFormat::BLUE . "{player}" . TextFormat::WHITE . " has left the match!");
        $hub = $this->getConfig()->get("hub", "MurderHub");
        if (!$this->getServer()->isLevelGenerated($hub)){
            $this->getServer()->getLogger()->error("The world named $hub doesn't exist. Change it in the configuration file.");
